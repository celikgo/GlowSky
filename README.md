...
make test                     # 83 tests: firewall, tools, slow-path + streaming, container runtime, gateway, agent loop, API, auth/tenancy, migrations, ADMET/docking backends, library I/O, run export
...
| **Library + SMILES/CSV/SDF I/O** *(Phase 1)* | `services/chemistry/io.py`, `apps/api/main.py` | Tenant-scoped libraries; import/export in SMILES/CSV/SDF (every structure firewalled, InChIKey-deduped with property-merge on re-import, bad rows reported not fatal); molecule diff with per-descriptor deltas |
...
| **Desktop app** *(Phase 1)* | `apps/desktop/` | **Tauri 2 + React + Vite + TS** desktop client themed in the Twitter **Dim** palette. The Design screen drives the agentic loop end-to-end (goal + seed → plan, candidate cards with **RDKit-JS 2D structure rendering**, explanation, trace, notebook/report export); the Library screen does projects + SMILES/CSV/SDF import/export against the I/O endpoints. See `apps/desktop/README.md` |
